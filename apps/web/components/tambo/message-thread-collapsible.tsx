...
import { SelectWorkspace } from "../workspaces/seletect-workspace";
...
export interface MessageThreadCollapsibleProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Controlled isOpen state - if provided, component becomes controlled */
  isOpen?: boolean;
  /** Callback for when open state changes - used with controlled isOpen */
 ...
 */
const useCollapsibleState = (defaultOpen = false, isControlled = false, currentOpen = false, onOpenChange?: (open: boolean) => void) => {
  ...
  }, [isControlled, currentOpen, onOpenChange]);
...
>(
  (
    ...
    const { isOpen: internalIsOpen, setIsOpen: setInternalIsOpen, shortcutText } =
      useCollapsibleState(defaultOpen, isControlled, controlledIsOpen || false, controlledOnOpenChange);
    ...
    return (
      ...
      >
        ...
        <Collapsible.Content>
          <div
            className={cn("flex flex-col", effectiveHeight ? "" : "h-[53vh]")}
            style={effectiveHeight ? { height: effectiveHeight } : undefined}
          >
            {/* Message thread content */}
            <ScrollableMessageContainer className="p-4">
              <ThreadContent variant={variant}>
                <ThreadContentMessages className="text-sm" />
              </ThreadContent>
