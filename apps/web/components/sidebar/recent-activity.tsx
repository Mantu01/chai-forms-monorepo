...
export function RecentActivity({ recentForms, recentComments }: RecentActivityProps) {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4 lg:px-6">
      <Card className="border border-zinc-800 backdrop-blur-md">
        <CardHeader>
        ...
        <CardContent className="space-y-3">
          {recentForms.length === 0 ? (
            ...
            recentForms.map((form) => (
              <div key={form.id} className="p-3 rounded-xl border border-zinc-800/80  flex flex-col gap-2">
                <div className="flex justify-between items-start">
                  <h4 className="text-xs font-semibold truncate">{form.title}</h4>
        ...
        <CardContent className="space-y-3">
          {recentComments.length === 0 ? (
            ...
            recentComments.map((comment) => (
              <div key={comment.id} className="p-3 rounded-xl border border-zinc-800/80  flex flex-col gap-1.5">
